...
export default function PlanPage() {
  ...
  const [planningMode, setPlanningMode] = useState<'short' | 'long'>('short');
    ...
    async function loadPlan() {
      try {
        ...
        for (const phase of PHASES) {
          const conv = await storage.getConversation(planId, phase.id);
  ...
  const sendInitialGreeting = async (phase: PlanPhase) => {
    ...
    try {
      ...
      // Save to storage
      const storage = getStorageAdapter();
      await storage.saveConversation(planId, phase, [assistantMessage]);
  ...
  const handlePhaseSelect = async (phase: PlanPhase) => {
    ...
    try {
      ...
      await storage.updatePlan(planId, { currentPhase: phase });
      setPlan(prev => prev ? { ...prev, currentPhase: phase } : null);

      // Load conversation for new phase
      const conversation = await storage.getConversation(planId, phase);
      setMessages(conversation);
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      // Save to storage
      const storage = getStorageAdapter();
      await storage.saveConversation(planId, currentPhase, updatedMessages);
