 ...
 * 
 * Security: Uses httpOnly session cookie set by middleware.
 * The session_id cookie is:
 * - HttpOnly (not accessible via JavaScript)
 * - Secure (HTTPS only in production)
 * - SameSite=Strict (prevents CSRF)
 * 
 * In production with REQUIRE_AUTH=true, this requires a valid auth session.
 * 
 * TODO: When adding real authentication, replace this with actual user ID from session
 */
function getAuthenticatedUserId(request: NextRequest): string | null {
  ...
  if (!/^session_\d+_[a-f0-9]{16,}$/.test(sessionId) && 
      !/^[a-f0-9-]{36}$/.test(sessionId)) {
    // Allow legacy format or UUID format
    if (!/^[\w-]+$/.test(sessionId)) {
      return null;
    }
  }
  
  // For now, use hardcoded dev user to match plan creation
  // This will be replaced with real auth when implemented
