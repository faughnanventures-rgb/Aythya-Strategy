...
const RATE_LIMIT_CONFIG = {
  requestsPerHour: parseInt(process.env.RATE_LIMIT_REQUESTS_PER_HOUR || '10', 10),
  requestsPerDay: parseInt(process.env.RATE_LIMIT_REQUESTS_PER_DAY || '50', 10),
};
...
export function checkRateLimit(userId: string): RateLimitResult {
  ...
  const hourEntry = rateLimits.get(hourKey);
  
  // Clean up expired entries
  if (hourEntry && hourEntry.resetAt < now) {
    rateLimits.delete(hourKey);
  }
  
  const currentHourEntry = rateLimits.get(hourKey) || {
    count: 0,
    resetAt: now + 3600000, // 1 hour
  };
  
  if (currentHourEntry.count >= RATE_LIMIT_CONFIG.requestsPerHour) {
    return {
      allowed: false,
      remaining: 0,
      resetIn: Math.ceil((currentHourEntry.resetAt - now) / 1000),
    };
  }
  
  // Update count
  currentHourEntry.count += 1;
  rateLimits.set(hourKey, currentHourEntry);
  
  return {
    allowed: true,
    remaining: RATE_LIMIT_CONFIG.requestsPerHour - currentHourEntry.count,
    resetIn: Math.ceil((currentHourEntry.resetAt - now) / 1000),
  };
}
...
function buildSystemPrompt(phase: PlanPhase, mode: PlanningMode = 'long'): string {
  ...
  return `${BASE_SYSTEM_PROMPT}${modeContext}\n\n${phasePrompts[phase]}`;
}
...
export async function chat(input: ChatInput): Promise<ChatResponse> {
  ...
  let systemPrompt = buildSystemPrompt(input.phase, mode);
