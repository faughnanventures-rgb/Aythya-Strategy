...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const planData = plan as { id: string; user_id: string; title: string; current_phase: string };
    
    // Double-check ownership (RLS should handle this, but defense in depth)
    if (planData.user_id !== userId) {
      console.warn(`[SECURITY] Plan ownership mismatch: user=${userId} plan_owner=${planData.user_id}`);
      return createErrorResponse(
        'Plan not found or access denied',
        404,
        'NOT_FOUND',
